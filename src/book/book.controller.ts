...
export class BookController {
  ...
  ): Promise<{ id: number; title: string; publishedDate: string }> {
    try {
      ...
      if (error instanceof BadRequestException)
        ...
        console.error(
          `Error processing request for book with Ttile ${createBookDto.title}:`,
          error,
        );
        throw new HttpException(
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async remove(
    @Param('id') removeBookDto: removeBookDto,
  ): Promise<{ message: string }> {
    try {
      const isRemoved = await this.BookService.remove(removeBookDto.id);
      return {
        message: `Book with ID ${removeBookDto.id} removed Successfuly`,
      };
    } catch (error) {
      if (error instanceof NotFoundException)
        throw new HttpException(
          `Book with ID ${removeBookDto.id} not Found`,
